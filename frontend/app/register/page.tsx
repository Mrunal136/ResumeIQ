                ...
                >
                    {displayError && (
                        <div className="mb-5 rounded-xl bg-warn-soft border border-warn/15 px-4 py-3 text-sm text-warn flex items-center justify-between">
                            ...
                            <button onClick={() => { setLocalError(""); clearError(); }} className="text-warn/50 hover:text-warn ml-2">✕</button>
                        </div>
                    )}
                    ...
                    <div className="space-y-4">
                        {/* Role Toggle */}
                        <div>
                            <label className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-text-muted">I am a</label>
                            <div className="flex gap-2">
                                <button type="button" onClick={() => setRole("candidate")}
                                    className={`flex-1 rounded-xl border py-2.5 text-sm font-medium transition-all ${role === "candidate" ? "border-primary bg-primary-soft text-primary" : "border-border bg-bg text-text-muted hover:border-primary/40"
                                        }`}>
                                    🎯 Job Seeker
                                </button>
                                <button type="button" onClick={() => setRole("recruiter")}
                                    className={`flex-1 rounded-xl border py-2.5 text-sm font-medium transition-all ${role === "recruiter" ? "border-primary bg-primary-soft text-primary" : "border-border bg-bg text-text-muted hover:border-primary/40"
                                        }`}>
                                    💼 Recruiter
                                </button>
                            </div>
                        ...
                        <div>
                            <label className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-text-muted">Full Name</label>
                            <div className="relative">
                                <HiOutlineUser className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-text-muted" />
                                <input
                                    type="text"
                                    value={name}
                        ...
                        <div>
                            <label className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-text-muted">Confirm Password</label>
                            <div className="relative">
                                <HiOutlineLockClosed className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-text-muted" />
                                <input
                                    type="password"
